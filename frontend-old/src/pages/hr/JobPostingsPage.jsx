...
import { getJobs, hrGetJobPipelineHealth, createJob, updateJob, adminGetJobs } from '../../api/index.js';
...
const FormFields = ({ form, setForm, weightsValid, positions = [] }) => {
  const field = (key) => ({
    value: form[key],
  ...
  const handlePositionChange = (event) => {
    ...
    const match = positions.find((p) => positionKey(p.title, p.level) === key);
    setForm(f => ({
      ...f,
  ...
  // If the form already has a (title, level) that isn't in the catalog (legacy
  // posting), surface that as a disabled "current value" option so the edit
  // form doesn't silently blank it on first render.
  const existsInCatalog = positions.some((p) => positionKey(p.title, p.level) === selectedPositionKey);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div>
        ...
        >
          ...
          {showLegacyOption ? (
            <option value={selectedPositionKey} disabled>
              {form.title}{form.level ? ` — ${form.level}` : ''} (not in catalog)
            </option>
          ) : null}
          {positions.map((p) => {
            const key = positionKey(p.title, p.level);
            return (
        ...
        <div style={{ fontSize: 11.5, color: 'var(--gray-500)', marginTop: 6 }}>
          Only positions from the catalog can be posted. Ask an admin to add a new one if it's missing.
        </div>
      </div>
...
export function HRJobPostingsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [jobData, healthData, positionData] = await Promise.all([
        getJobs(),
  ...
  return (
    <div className="hr-page-shell">
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title={t('Create Job Posting')} maxWidth={600}>
        <FormFields form={form} setForm={setForm} weightsValid={weightsValid} positions={positions} />
