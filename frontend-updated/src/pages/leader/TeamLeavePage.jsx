...
import { LeaderPortalLayout, Btn, Badge, Spinner, Textarea, useToast } from '../../components/shared/index.jsx';
import { useLanguage } from '../../context/LanguageContext';
import { Plane, Check, X, User, Paperclip, CalendarDays } from 'lucide-react';
    ...
    <LeaderPortalLayout>
      ...
      {visible.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 18 }}>
          {visible.map(req => <LeaveCard key={req.leaveRequestID} request={req} onReviewed={load} t={t} toast={toast} />)}
        </div>
      )}
    </LeaderPortalLayout>
  );
...
function LeaveCard({ request, onReviewed, t, toast }) {
  ...
  const review = async (action) => {
    setBusy(true);
    try {
      await reviewTeamLeaveRequest(request.leaveRequestID, { action, reviewNotes: note.trim() });
