...
function LeaderOwnedShell({ titleKey, variant = 'overview', children }) {
  const { t } = useLanguage();
  const { user, resolvePath, loading: authLoading } = useAuth();
      ...
      <div className="hr-surface-card workspace-shell-card workspace-shell-leader" style={{ maxWidth: 1280, margin: '0 auto 18px', padding: '18px 20px' }}>
        ...
        {loading || authLoading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12, marginBottom: 14 }}>
            {metrics.map((card) => (
              <div key={card.label} className="workspace-focus-card" style={{ background: 'var(--bg-surface)' }}>
...
function LeaderCoachingHub() {
  const { t } = useLanguage();
  const { resolvePath, loading: authLoading } = useAuth();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
        ...
        <>
          <div className="workspace-journey-strip" style={{ marginBottom: 16 }}>
            {[
              ...
              { label: t('Delivery Risk'), value: brief.deliveryRisk, note: t('Blocked and overdue work signals now visible to the leader.'), accent: brief.deliveryRisk ? 'var(--pink-400)' : '#22C55E' },
            ...
            <div className="hr-surface-card" style={{ padding: 16, background: '#FCFCFD' }}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                {brief.plays.map((play) => (
                  <div key={play.title} className="workspace-focus-card" style={{ background: 'var(--bg-surface)' }}>
                    <div className="workspace-focus-label">{play.title}</div>
                    <div className="workspace-focus-note">{play.note}</div>
                  </div>
                ))}
                <div className="workspace-focus-card" style={{ background: 'var(--bg-surface)' }}>
