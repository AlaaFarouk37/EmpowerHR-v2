...
export function HRBenefitsPage() {
  ...
  const loadBenefits = async () => {
    setLoading(true);
    try {
      const data = await hrGetBenefits();
      setBenefits(Array.isArray(data) ? data : []);
    } catch (error) {
      toast('Failed to load benefits data', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredBenefits = useMemo(() => {
    return benefits.filter(b => {
      const matchesSearch = b.benefitName?.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesType = activeType === 'All Benefits' || b.benefitType === activeType;
      return matchesSearch && matchesType;
    });
  }, [benefits, searchQuery, activeType]);

  const categories = useMemo(() => {
    const types = new Set(benefits.map(b => b.benefitType).filter(Boolean));
    return ['All Benefits', ...Array.from(types)];
  }, [benefits]);

  const benefitStats = useMemo(() => {
    return [
      { label: 'Global Enrollment', value: '94.2%', icon: Users, color: '#1E293B', bg: '#F8FAFC' },
      { label: 'Operational Cost', value: '$124.8k', icon: DollarSign, color: 'var(--red-600)', bg: 'var(--red-50)' },
      { label: 'Avg Node Value', value: '$480', icon: Heart, color: 'var(--red-800)', bg: 'var(--red-50)' },
      { label: 'Active Claims', value: '18', icon: Activity, color: 'var(--pink-600)', bg: 'var(--pink-50)' },
    ];
  }, []);
  ...
  if (loading) return (
    <div style={{ height: '80vh', display: 'grid', placeItems: 'center' }}>
       <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 900, color: '#1E293B', letterSpacing: '0.1em' }}>SYNCHRONIZING BENEFITS GRID...</div>
       </div>
    </div>
  );
  ...
  return (
    <div className="page-content animate-in" style={{ background: '#F8FAFC', minHeight: '100vh', padding: '40px 60px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 48 }}>
        <div>
           <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              ...
              <h1 style={{ fontSize: 32, fontWeight: 900, color: '#1E293B', margin: 0, letterSpacing: '-0.02em' }}>Benefits & Wellness Command</h1>
           </div>
           <p style={{ fontSize: 14, color: '#94A3B8', fontWeight: 600 }}>Manage organizational benefit programs, wellness telemetry, and compensation structures.</p>
        </div>
        ...
        >
           <Zap size={18} style={{ marginRight: 8 }} /> {t('Initialize New Plan')}
        </Btn>
      </div>

      {/* Wellness Telemetry Strip */}
      ...
      <div style={{ background: '#fff', padding: '16px 24px', borderRadius: 24, border: '1.5px solid #F1F5F9', marginBottom: 32, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)' }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
           <div style={{ position: 'relative' }}>
              <select 
                ...
                style={{ height: 44, padding: '0 40px 0 16px', borderRadius: 12, border: '1.5px solid #F1F5F9', background: '#F8FAFC', fontSize: 13, fontWeight: 800, color: '#1E293B', outline: 'none', appearance: 'none', minWidth: 180 }}
              >
           ...
           <div style={{ position: 'relative' }}>
              ...
              <input 
                type="text" 
                placeholder={t('Search benefit programs...')}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                style={{ height: 44, padding: '0 16px 0 48px', borderRadius: 12, border: '1.5px solid #F1F5F9', background: '#F8FAFC', fontSize: 13, fontWeight: 600, width: 300, outline: 'none' }} 
              />
           </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
           <Btn variant="secondary" style={{ borderRadius: 12, height: 44, fontWeight: 800 }}>
              <Filter size={16} style={{ marginRight: 8 }} /> {t('Strategic Filters')}
           </Btn>
           <Btn variant="outline" style={{ borderRadius: 12, height: 44, fontWeight: 800 }}>
              <Globe size={16} style={{ marginRight: 8 }} /> {t('Audit Logs')}
           </Btn>
        </div>
      </div>

      {/* High-Fidelity Benefit Ledger */}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#F8FAFC', borderBottom: '1.5px solid #F1F5F9' }}>
              {['Benefit Program', 'Classification', 'Enrollment Load', 'Cost / Node', 'Status', 'Actions'].map(h => (
          ...
          <tbody>
            {filteredBenefits.map((benefit, idx) => {
              ...
              const enrollmentLoad = Math.min((enrolledCount / 250) * 100, 100);
              
              return (
                <tr key={idx} style={{ borderBottom: '1px solid #F1F5F9', transition: 'background 0.2s' }} className="program-row">
                  <td style={{ padding: '24px 32px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 15, fontWeight: 900, color: '#1E293B' }}>{benefit.benefitName}</div>
                        <div style={{ fontSize: 11, color: '#94A3B8', fontWeight: 800 }}>{benefit.provider || 'Internal Program'}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '24px 32px' }}>
                     <Badge label={benefit.benefitType} color="indigo" />
                  </td>
                  <td style={{ padding: '24px 32px' }}>
                     <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ flex: 1, height: 6, background: '#F1F5F9', borderRadius: 10, overflow: 'hidden', minWidth: 100 }}>
                           <div style={{ width: `${enrollmentLoad}%`, height: '100%', background: 'var(--red-600)', borderRadius: 10 }} />
                        </div>
                        <span style={{ fontSize: 13, fontWeight: 900, color: '#1E293B' }}>{enrolledCount} Nodes</span>
                     </div>
                  </td>
                  <td style={{ padding: '24px 32px' }}>
                     <div style={{ fontSize: 14, fontWeight: 900, color: '#1E293B' }}>
                        ${benefit.monthlyCost || '120'} <span style={{ fontSize: 11, color: '#94A3B8', fontWeight: 700 }}>/ month</span>
                     </div>
                  </td>
                  <td style={{ padding: '24px 32px' }}>
                     <Badge 
                      label={benefit.status || 'Active'} 
                      color={benefit.status === 'Inactive' ? 'red' : 'green'} 
                     />
                  </td>
                  <td style={{ padding: '24px 32px' }}>
                    <div style={{ display: 'flex', gap: 10 }}>
                       <button className="action-btn" title="Adjust Parameters"><Settings size={18} /></button>
                       <button className="action-btn" title="Add Participant"><UserPlus size={18} /></button>
                       <button className="action-btn" title="View Analytics"><TrendingUp size={18} /></button>
                    </div>
                  </td>
                </tr>
              );
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .program-row:hover { background: #FBFBFF; }
        .action-btn { 
