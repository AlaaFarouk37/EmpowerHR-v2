    ...
    <div className="hr-page-shell" style={{ maxWidth: 1180, margin: '0 auto', padding: '40px 32px 80px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          ...
          { label: t('Pending Acknowledgment'), value: stats.pending, accent: 'var(--pink-400)' },
          { label: t('Acknowledged'), value: stats.acknowledged, accent: 'var(--red-800)' },
        ].map((card) => (
            ...
            {reviewTypePressureMap.length === 0 ? (
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                {reviewTypePressureMap.map((item) => (
                  <div key={item.type} style={{ border: '1px solid var(--border-primary)', borderRadius: 14, padding: '12px 14px', background: 'var(--bg-surface)' }}>
              ...
              <div key={review.reviewID} className="hr-surface-card" style={{ padding: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--pink-400)' }}>{'★'.repeat(rating)}{'☆'.repeat(5 - rating)}</div>
