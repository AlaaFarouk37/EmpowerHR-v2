...
export function EmployeeTasksPage() {
  ...
  // Once the TL approves a task it becomes 'Done' — hide those from the employee.
  ...
  const stats = useMemo(() => ({
    ...
    overdue: visibleTasks.filter((task) => daysUntilDue(task.dueDate) < 0).length,
    dueSoon: visibleTasks.filter((task) => {
      const days = daysUntilDue(task.dueDate);
      return days >= 0 && days <= 3;
    }).length,
  }), [visibleTasks]);

  // Overdue (past due) or due soon (within 3 days), most urgent first.
  const taskFocusQueue = useMemo(() => {
    const priorityRank = { High: 3, Medium: 2, Low: 1 };
    return [...visibleTasks]
      .filter((task) => !isInactive(task) && daysUntilDue(task.dueDate) <= 3)
    ...
    <div className="page-content animate-in" style={{ background: '#F8FAFC', minHeight: '100vh', padding: '40px 60px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, marginBottom: 48 }}>
        ...
        <div style={{ padding: '24px', borderRadius: 28, background: '#fff', border: '1.5px solid #F1F5F9', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', display: 'flex', alignItems: 'center', gap: 20 }}>
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 900, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('Priority Tasks')}</div>
            <div style={{ fontSize: 24, fontWeight: 900, color: '#1E293B' }}>{stats.highPriority}</div>
          </div>
        </div>
        <div style={{ padding: '24px', borderRadius: 28, background: '#fff', border: '1.5px solid #F1F5F9', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', display: 'flex', alignItems: 'center', gap: 20 }}>
          <div style={{ width: 48, height: 48, borderRadius: 14, background: '#FEF2F2', color: '#DC2626', display: 'grid', placeItems: 'center' }}>
            <AlertTriangle size={22} />
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 900, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('Overdue')}</div>
            <div style={{ fontSize: 24, fontWeight: 900, color: '#1E293B' }}>{stats.overdue}</div>
          </div>
        </div>
        <div style={{ padding: '24px', borderRadius: 28, background: '#fff', border: '1.5px solid #F1F5F9', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)', display: 'flex', alignItems: 'center', gap: 20 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 24, fontWeight: 900, color: '#1E293B' }}>{stats.dueSoon}</div>
          </div>
        </div>
      </div>
        ...
        <div style={{ display: 'grid', gap: 24, alignContent: 'start', position: 'sticky', top: 24 }}>
          <div style={{ background: '#fff', borderRadius: 32, padding: '32px', border: '1.5px solid #F1F5F9', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.03)' }}>
             <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
               ...
               <div style={{ fontSize: 14, fontWeight: 900, color: '#1E293B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('Focus Queue — Tasks Near Overdue')}</div>
             </div>
             {taskFocusQueue.length === 0 ? (
               ...
               <div style={{ display: 'grid', gap: 12 }}>
                 {taskFocusQueue.map(task => {
                   const overdue = daysUntilDue(task.dueDate) < 0;
                   return (
                     <div key={task.taskID} style={{ padding: '14px 16px', borderRadius: 16, background: '#F8FAFC', border: '1px solid #F1F5F9', display: 'flex', alignItems: 'center', gap: 12 }}>
                       <AlertCircle size={18} style={{ color: '#DC2626', flexShrink: 0 }} />
                       <div style={{ minWidth: 0 }}>
                         <div style={{ fontSize: 14, fontWeight: 800, color: '#1E293B', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{task.title}</div>
                         <div style={{ fontSize: 12, fontWeight: 800, color: overdue ? '#DC2626' : '#EA580C', marginTop: 2 }}>{getDueWindowLabel(task.dueDate, t)}</div>
                       </div>
                     </div>
                   );
                 })}
