      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 28px 0' }}>
        ...
        <div className="hr-surface-card" style={{ padding: 24, borderRadius: 28, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12 }}>
            <div style={{ border: '1px solid #E7EAEE', borderRadius: 18, padding: '14px 16px', background: 'var(--bg-surface)' }}>
            ...
            <div style={{ border: '1px solid #E7EAEE', borderRadius: 18, padding: '14px 16px', background: 'var(--bg-surface)' }}>
              ...
              <div style={{ display: 'grid', gap: 8 }}>
                {(featuredJob ? getRoleHighlights(featuredJob) : ['Skill-first role feed', 'Clean apply flow', 'Live tracker updates']).map((item) => (
                  <div key={item} style={{ padding: '9px 10px', borderRadius: 12, background: 'var(--bg-secondary)', border: '1px solid #E7EAEE', fontSize: 12.5, color: 'var(--gray-700)' }}>
                    {t(item)}
                  </div>
                ))}
              </div>
              ...
              {interviewReadinessItems.length === 0 ? (
                ...
                <div style={{ display: 'grid', gap: 10 }}>
                  {interviewReadinessItems.map((item) => (
                    <div key={item.id} style={{ padding: '12px 14px', borderRadius: 14, background: 'var(--bg-secondary)', border: '1px solid #E7EAEE' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.2fr) minmax(0, .9fr) auto', gap: 12, alignItems: 'center', marginBottom: 18 }}>
            <div>
              <input
                ...
                style={{ width: '100%', padding: '13px 14px', border: '1px solid #E7EAEE', borderRadius: 14, fontSize: 13.5, outline: 'none', background: isSignedInCandidate ? 'var(--bg-secondary)' : '#fff' }}
              />
          ...
          {!trackerLoaded ? (
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              {applications.map((application) => {
                ...
                return (
                  <div key={application.id} style={{ border: '1px solid #E7EAEE', borderRadius: 22, padding: 18, background: 'var(--bg-surface)' }}>
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              fontSize: 12,
                              fontWeight: 700,
                              background: isCurrent ? 'rgba(232,50,26,.12)' : isComplete ? '#FEEDEA' : 'var(--bg-secondary)',
                    ...
                    <div style={{ marginBottom: 12, padding: '10px 12px', borderRadius: 14, background: 'var(--bg-secondary)', border: '1px solid #E7EAEE' }}>
                    ...
                    {history.length ? (
                      <div style={{ marginBottom: 12, padding: '10px 12px', borderRadius: 14, background: 'var(--bg-surface)', border: '1px solid #E7EAEE' }}>
                        ...
                        <div style={{ display: 'grid', gap: 8 }}>
                          {history.map((entry, index) => (
                            <div key={`${application.id}-history-${index}`} style={{ padding: '8px 10px', borderRadius: 10, background: 'var(--bg-secondary)', border: '1px solid #EEF2F6' }}>
      ...
      <Modal open={showApply} onClose={() => { setShowApply(false); setResult(null); }} title={result ? t('Pipeline Results') : `${t('Apply for')} ${selected?.title || ''}`} maxWidth={580}>
        {result ? (
          <div>
            <div style={{ textAlign: 'center', padding: '20px 0', borderBottom: '1px solid #F3F4F6', marginBottom: 20 }}>
              ...
              {result.tracking_code ? (
                <div style={{ marginTop: 14, padding: '12px 14px', borderRadius: 14, background: 'var(--bg-secondary)', border: '1px solid #E7EAEE' }}>
          ...
          <div>
            ...
            {selected ? (
              <div style={{ marginBottom: 16, padding: '14px 16px', borderRadius: 16, background: 'var(--bg-secondary)', border: '1px solid #E7EAEE' }}>
