...
const PATH_PERMISSION_MAP = {
  ...
  '/hr/profile': 'hr.workspace.access',
...
const normalizeUserData = (payload = {}, fallbackEmail = '') => ({
  ...
  ui_theme: payload.ui_theme || payload.uiTheme || 'intelligence',
...
export function AuthProvider({ children }) {
  ...
  const [portalView, setPortalViewState] = useState(() => localStorage.getItem("empowerhr_portal_view") || "manager");

  const setPortalView = useCallback((view) => {
    setPortalViewState(view);
    localStorage.setItem("empowerhr_portal_view", view);
  }, []);
  ...
  const updateAccountPreferences = useCallback(async (preferences) => {
    ...
    setUser(prev => {
      const nextUser = { ...prev, ...normalizeUserData(data, prev?.email) };
      persistSessionUser(nextUser);
      return nextUser;
    });
  }, []);
